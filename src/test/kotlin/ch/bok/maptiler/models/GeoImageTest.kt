...
class GeoImageTest : GeoImageFixtures {
    ...
    val orthoPhotoImage = anOrthoPhotoImage()

    @Test
    fun `x GSD should equal to y GSD`() {
        assertEquals(orthoPhotoImage.getXGSD(), orthoPhotoImage.getYGSD(), 1e-12)
    }

    @Test
    fun `should get GSD UTM34N`() {
        val got = orthoPhotoImage.getGSD()
    ...
    inner class CompanionObj {
        ...
        fun `should read dimensions from tiff`() {
            val got = orthoPhotoImage.dimensions
    ...
    fun `image ratio is coherent between dimension and measures`() {
         ...
         */
        val imageUTM = anOrthoPhotoImage(file = "odm_orthophoto_384.tif")
    ...
    fun `positionToCoords`(
        ...
        val got = anOrthoPhotoImage().positionToCoords(pos).toCrs(coords.crs)
    ...
    fun `coordsToPosition`(
        ...
        val got = anOrthoPhotoImage().coordsToPosition(coords)
    ...
    companion object : GeoImageFixtures {
        ...
        fun positionToCoordsData() = listOf(
            Arguments.of(Position(0, 0), aNWCornerWGS84()),
            Arguments.of(Position(0, 0), aNWCornerUTM34M()),
            Arguments.of(Position(1375, 1407), aSECornerWGS84()),
            Arguments.of(Position(1375, 1407), aSECornerUTM34M()),
        )
    }
}